...
public class b_GenerateLatinAmerica : MonoBehaviour {
	...
	IEnumerator  delayCall () {
		...
		for (int i = 0; i < numberOfObjects; i++) {
			Vector3 position = Random.onUnitSphere * 2.4f; // Change this value depending on how large the unit circle should be
			Instantiate (continent_group, position, Quaternion.identity);
			yield return new WaitForSeconds (0.03f);
		}

		if (numberOfObjects == numberOfObjects) {
			print ("Latin America complete " + Time.time);
		}
	}
}
